...
class SignUpActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        checkDuplicateButton.setOnClickListener {
            ...
            // API 호출
            RetrofitInstance.api.checkIdentityDuplicate(inputId).enqueue(object : Callback<DuplicateCheckResponse> {
                override fun onResponse(call: Call<DuplicateCheckResponse>, response: Response<DuplicateCheckResponse>) {
                            ...
                            .enqueue(object : Callback<LoginResponse> {
                                override fun onResponse(
                                    ...
                                    if (loginResponse.isSuccessful && data?.accessToken != null) {
                                        ...
                                        Toast.makeText(this@SignUpActivity, "회원가입 및 로그인 성공!", Toast.LENGTH_SHORT).show()

                                        val intent = Intent(this@SignUpActivity, UserDetailActivity::class.java)
                                        startActivity(intent)
                                        finish()
                                    } else {
                                        Toast.makeText(this@SignUpActivity, "자동 로그인 실패: ${loginResponse.code()}", Toast.LENGTH_SHORT).show()
                                    }
                                }

                                override fun onFailure(call: Call<LoginResponse>, t: Throwable) {
                                    Toast.makeText(this@SignUpActivity, "자동 로그인 네트워크 오류: ${t.message}", Toast.LENGTH_SHORT).show()
                                }
                            })
                    } else {
