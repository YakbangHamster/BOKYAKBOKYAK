...
class MedicineAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return if (viewType == VIEW_TYPE_ITEM) {
            ...
            val view = LayoutInflater.from(parent.context)
                .inflate(footerLayoutRes, parent, false)
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (holder is ItemViewHolder && position < items.size) {
            ...
            // 식약처 이미지 URL로 약 사진 표시
            if (!item.image.isNullOrBlank()) {
                Glide.with(holder.itemView.context)
                    .load(item.image)
                    .override(144, 144)                 // 픽셀 단위로 리사이즈
                    .centerCrop()                       // 가운데 기준으로 잘라서 채우기
                    .placeholder(R.drawable.hamster)
                    .error(R.drawable.hamster)
                    .into(holder.hamster)
            } else {
                holder.hamster.setImageResource(R.drawable.hamster)
            }
